...
export const Body = () => {
	...
	const { data, loading } = useFetch(getLocation.setParams({ location, year: new Date().getFullYear() }));
	...
	return (
		<Center my="xl">
			<Stack miw="20rem" maw="95vw">
				...
				{!loading && data && <Title order={1}>{locations?.find((l) => l.id.toString() === location)?.name}</Title>}

				{!loading && !data && <Title order={3}>{t("errorDisplay")}</Title>}

				{!loading && data && <TideList data={data} date={date} dayCount={dayCount} />}
			</Stack>
		</Center>
	);
};
